...
業務で、`Formik`を利用した際、フォーム選択された値を表示する処理があったのですが、フォームの項目が膨大で、割と処理も遅い印象だったのですが、 `React Hook Form`へ切り替えた際、速いなと。しかも、最近、v6からv7にアップデートした際、更に処理速度が上がっているなあと体感できるほどで。
